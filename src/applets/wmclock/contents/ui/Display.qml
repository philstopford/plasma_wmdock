...
Item {
    ...
    Canvas {
        ...
        // Bezier control points intentionally extend beyond some visible endpoints
        // so the rendered curves land at the right apparent height and width.
        ...
        function paintNixie() {
            ...
            for (let i = 0; i < 5; i++) {
                ...
                const innerTubeW = (tubeStyle === "slim")
                    ...
                    : tW
                const effGlyphW  = Math.max(4, Math.floor(innerTubeW * 0.76))
                // Taller, slimmer glyph cell to better match real nixie cathode proportions
                // while still respecting the selected tube's inner width.
                const effGlyphH  = Math.max(6, Math.min(Math.floor(tubeH * 0.90),
                                                          Math.floor(effGlyphW * 2.55)))
                // Slightly slimmer wire so the cathodes stay readable without turning
                // into over-thick blobs once glow/highlight passes are added.
                const wireW      = Math.max(1.4, effGlyphW * 0.09)
